...
export default function AssignDriverButton({
  ...
  declinedDriverName?: string | null;
    ...
    <div className={`relative ${open ? 'z-50' : 'z-10'}`} ref={wrapperRef}>
      {isDeclined ? (
        <div className="flex flex-col gap-1.5 items-start">
          {shownDeclinedName && (
            <span className="text-[10px] text-muted-foreground truncate max-w-[140px]" title={shownDeclinedName}>
              Was: {shownDeclinedName}
            </span>
          )}
        ...
        <>
          <button
            onClick={handleOpen}
            disabled={assigning || assignmentLocked}
            ...
            title={assignmentLocked ? 'This ride is already accepted and cannot be reassigned.' : undefined}
          >
            {assigning ? (
            ...
            {assigning ? 'Assigning…' : assignmentLocked ? 'Locked' : 'Assign Driver'}
          </button>
          {dropdown}
        </>
      )}
